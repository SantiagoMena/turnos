...
$this->registerJs(
    ...
    $( '#turno-fecha' ).change(function() {
        $('#horarios').html('');
        $.get( \"$url?fecha=\"+$( '#turno-fecha' ).val(), function( data ) {
          $('#horarios').html(data);
        });
    });
...
<div class="turno-form">
    ...
    <?= 
     $form->field($model, 'servicios')->widget
        ...
        //         'name' => 'examenes',
        //         'id'=>'examenes',
        //         'data' =>ArrayHelper::map(Servicio::find()->joinWith('categoria')->select(['categoria.nombre AS categoria_nombre','examen.nombre AS examen_nombre'])->all(), 'id', 'categoria_nombre','examen_nombre'),
        //         'options' => [
